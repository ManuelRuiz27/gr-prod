...
export const handlers = [
  ...
  http.get('*/api/v1/me/events/:eventId/group', () => json({ members: getDemoState().group_members, products: getDemoState().products })),
  http.post('*/api/v1/me/events/:eventId/group-members', async ({ request }) => { const body = await request.json() as { full_name?: string; product_type?: 'ADULT' | 'CHILD' | 'NO_DINNER' }; return json(addDemoGroupMember(body.full_name || 'Integrante demo', body.product_type), 201); }),
  http.patch('*/api/v1/me/events/:eventId/group-members/:memberId', () => json(getDemoState())),
  http.post('*/api/v1/me/events/:eventId/contract-line-items/quote', async ({ request }) => { await request.json(); return json({ quote_id: 'quote-product-demo', expires_at: '2027-04-30T23:59:59Z' }, 201); }),
  http.post('*/api/v1/me/events/:eventId/contract-line-items', () => json(getDemoState(), 201)),
  ...
  http.post('*/api/v1/admin/events/:eventId/graduates/:membershipId/cancel', async ({ request }) => { const body = await request.json() as { quote_id: string; reason: string }; return json(cancelDemoMembership(body.reason)); }),
  ...['financial', 'portfolio', 'payments', 'payment-submissions', 'tables', 'meals', 'thermos', 'cash-cuts'].map((report) => http.get(`*/api/v1/admin/events/:eventId/reports/${report}`, () => { const state = getDemoState(); return json({ membership_status: state.membership_status, payments: state.payment_plan, allocations: state.payment_allocations, seating: state.seating, meals: state.meals, thermo: state.thermo }); })),
