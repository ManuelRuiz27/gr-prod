...
export const GraduatePaymentsScreen: React.FC<GraduatePaymentsScreenProps> = ({ graduateId = 'grad-andrea-martinez' }) => {
  ...
  const [step, setStep] = useState<'closed' | 'amount' | 'method' | 'details' | 'success'>('closed');
  ...
  return <div className="w-full max-w-5xl mx-auto flex flex-col gap-7 font-sans animate-fadeIn pb-12">
    ...
    {next && <section aria-label="Próximo abono" className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      ...
      <Button variant="primary" size="md" onClick={() => { setAmount(String(next.amount)); setMethod(null); setFolio(''); setStep('amount'); }}>Abonar</Button>
    </section>}
    ...
    <Modal
      ...
      title={
        step === 'amount' ? '¿Cuánto quieres abonar?' :
        step === 'method' ? '¿Cómo quieres pagar?' :
        step === 'details' ? (method === 'Mercado Pago' ? 'Pago en línea con Mercado Pago' : 'Datos para ' + (method ?? 'pago')) :
        'Comprobante enviado'
      }
      description={
        step === 'amount' && next ? 'Mínimo actual: ' + currency(next.amount) :
        step === 'method' ? 'Monto a abonar: ' + currency(parsedAmount) :
        step === 'details' ? 'Monto: ' + currency(parsedAmount) :
    ...
    >
      {step === 'amount' && (
        <div className="space-y-4">
          <Input label="Monto" value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" />
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setStep('closed')}>Cancelar</Button>
            <Button variant="primary" disabled={!amountIsValid} onClick={() => setStep('method')}>Continuar</Button>
          </div>
        </div>
      )}
      {step === 'method' && (
        <div className="space-y-3">
          {['Mercado Pago', 'Reportar transferencia', 'Reportar depósito'].map((option) => (
            <label key={option} className="flex cursor-pointer items-center gap-3 py-2 text-sm text-silver-100">
              <input type="radio" name="payment-method" aria-label={option} value={option} checked={method === option} onChange={() => setMethod(option)} />
              {option}
            </label>
          ))}
          <p className="pt-2 text-xs text-silver-400">{method ? 'Continuarás con ' + method + '.' : 'Selecciona un método para continuar.'}</p>
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setStep('amount')}>Atrás</Button>
            <Button variant="primary" disabled={!method} onClick={() => setStep('details')}>Continuar</Button>
          </div>
        </div>
      )}
      {step === 'details' && method === 'Mercado Pago' && (
        <div className="space-y-4">
          <p className="text-sm text-silver-200">Serás redirigido a la pasarela segura de Mercado Pago para realizar tu abono de {currency(parsedAmount)}.</p>
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setStep('method')}>Atrás</Button>
            <Button variant="primary" onClick={() => setStep('success')}>Proceder al pago</Button>
          </div>
        </div>
      )}
      {step === 'details' && method !== 'Mercado Pago' && (
        <div className="space-y-4">
          <div className="rounded-lg bg-obsidian-900/60 p-3 text-xs text-silver-300 space-y-1">
            <p className="font-semibold text-silver-100">Cuenta de depósito / transferencia:</p>
            <p>Banco: BBVA</p>
            <p>Beneficiario: Graduaciones GR</p>
            <p>CLABE: 012180001234567890</p>
          </div>
          <Input label="Folio o número de referencia" value={folio} onChange={(e) => setFolio(e.target.value)} placeholder="Ej. 12345678" />
          <div className="flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setStep('method')}>Atrás</Button>
            <Button variant="primary" disabled={!folio.trim()} onClick={() => setStep('success')}>Enviar comprobante</Button>
          </div>
        </div>
      )}
      {step === 'success' && (
        <div className="space-y-4">
          <p className="text-sm text-silver-200">
            {method === 'Mercado Pago'
              ? `Pago registrado por ${currency(parsedAmount)}. Tu comprobante digital está en proceso de validación.`
              : `Hemos registrado tu comprobante con referencia ${folio} por ${currency(parsedAmount)}. Se revisará administrativamente.`}
          </p>
          <div className="flex justify-end">
            <Button variant="primary" onClick={() => setStep('closed')}>Aceptar</Button>
          </div>
        </div>
      )}
    </Modal>
