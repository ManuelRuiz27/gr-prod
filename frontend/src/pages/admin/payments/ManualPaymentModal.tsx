...
export interface ManualPaymentModalProps {
  ...
  onPaymentRegistered?: (data: ManualPaymentSubmitData) => void;
}
...
const ManualPaymentForm: React.FC<ManualPaymentFormProps> = ({
  ...
  const defaultGradId = initialGraduateId || mockGraduatesList[0]?.id || 'grad-andrea-martinez';
  const defaultPlan = mockPaymentPlansMap[defaultGradId];
  const defaultInst =
  ...
  const selectedGraduate = mockGraduatesList.find((g) => g.id === selectedGradId);
  ...
  const graduateOptions = mockGraduatesList.map((g) => ({
    value: g.id,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      {/* Evidence attachment (Dropzone simulation) */}
      <div className="flex flex-col gap-1.5">
        <span className="text-xs font-semibold text-content-primary">Evidencia de pago</span>
        <div
          onClick={() => setEvidenceFileName(evidenceFileName ? '' : 'comprobante_deposito_firmado.pdf')}
          className={`
        ...
        >
          <Icon
            ...
            size={20}
            className={evidenceFileName ? 'text-status-success' : 'text-content-muted'}
          />
          {evidenceFileName ? (
            <span className="text-xs font-semibold text-status-success">
              Archivo adjunto: {evidenceFileName} (clic para quitar)
            </span>
          ) : (
            <span className="text-xs text-content-secondary">
              Haz clic para adjuntar comprobante (PDF, JPG o PNG)
            </span>
          )}
        </div>
      </div>
      ...
      <div className="p-3 bg-surface-low rounded-xl flex items-start gap-2.5 text-xs text-content-secondary border border-surface-high">
        ...
        <p className="leading-relaxed">
          Este pago se registrará inmediatamente en el historial de{' '}
          <strong className="text-navy-900">{selectedGraduate?.fullName || 'el graduado'}</strong> y
          actualizará su avance financiero general.
        </p>
      </div>
...
export const ManualPaymentModal: React.FC<ManualPaymentModalProps> = ({
  ...
  // Step 2: UX-30 Pago Registrado Success View
  if (submittedData) {
    return (
      <Modal isOpen={isOpen} onClose={handleClose} size="sm">
        <div className="flex flex-col items-center text-center p-2">
          {/* Green Check Icon */}
          <div className="w-16 h-16 rounded-full bg-status-success-bg flex items-center justify-center text-status-success mb-4 animate-scaleUp">
            <Icon name="check" size={32} />
          </div>

          <h2 className="text-xl font-bold font-display text-navy-900">Pago registrado</h2>
          <p className="text-xs text-content-secondary mt-1">
            El movimiento quedó registrado en el historial.
          </p>

          {/* Amount Highlight Box */}
          <div className="w-full bg-surface-low rounded-2xl p-4 my-5 flex items-center justify-between border border-surface-high">
            <span className="text-xs font-semibold text-content-secondary">Monto registrado</span>
          ...
          {/* Detail List */}
          <div className="w-full space-y-2 text-xs border-t border-surface-low pt-3 mb-6">
  ...
  // Step 1: UX-29 Registrar Pago Manual Form
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
