...
import { acceptDemoContract, addDemoGroupMember, assignDemoTable, cancelDemoMembership, selectDemoMeal, transitionDemoThermo } from './actions';
...
export function DemoFlowPanel({ flow }: { flow: 'contract' | 'group' | 'seating' | 'meals' | 'thermo' | 'cancellation' | 'reports' }) {
  ...
  const error = (action: () => void) => { try { action(); } catch (reason) { window.alert(String(reason)); } };
  ...
  return <Card className="p-3 border border-gold-500/30 bg-obsidian-900/80 text-xs" data-testid={`demo-flow-${flow}`}>
    ...
    {flow === 'contract' && <div className="mt-2 flex items-center justify-between gap-2"><span>Contrato: {state.contract_status}</span><Button size="sm" variant="secondary" disabled={state.contract_status !== 'PENDING_ACCEPTANCE'} onClick={acceptDemoContract}>Aceptar contrato</Button></div>}
    {flow === 'group' && <div className="mt-2 flex items-center justify-between gap-2"><span>{state.group_members.length} integrantes · {state.products.map((item) => `${item.label} ×${item.quantity}`).join(', ')}</span><Button size="sm" variant="secondary" onClick={() => addDemoGroupMember('Invitado demo')}>Agregar integrante</Button></div>}
    {flow === 'seating' && <div className="mt-2 flex items-center justify-between gap-2"><span>{state.seating.financially_eligible && state.seating.deadline_open ? occupancy : 'Selección bloqueada por condición del evento'}</span><Button size="sm" variant="secondary" disabled={!firstMember} onClick={() => error(() => assignDemoTable(firstMember.id, 'tbl-25'))}>Asignar a Mesa 25</Button></div>}
    {flow === 'meals' && <div className="mt-2 flex items-center justify-between gap-2"><span>{state.meals.deadline_open ? `${state.meals.pending_count} platillos pendientes` : 'Fecha límite cerrada: solo lectura'}</span><Button size="sm" variant="secondary" disabled={!state.meals.deadline_open || !firstMember} onClick={() => error(() => selectDemoMeal(firstMember.id, 'meal-vegetarian'))}>Elegir vegetariano</Button></div>}
    {flow === 'thermo' && <div className="mt-2 flex items-center justify-between gap-2"><span>Termo: {state.thermo.status}</span>{nextThermo && <Button size="sm" variant="secondary" onClick={() => transitionDemoThermo(nextThermo)}>Cambiar a {nextThermo}</Button>}</div>}
    {flow === 'cancellation' && <div className="mt-2 flex items-center justify-between gap-2"><span>Quote {state.cancellation_quote.quote_id}: reembolso ${state.cancellation_quote.refund_due} · adeudo ${state.cancellation_quote.remaining_due}</span><Button size="sm" variant="danger" disabled={state.membership_status === 'CANCELLED'} onClick={() => error(() => cancelDemoMembership('Cancelación de demostración'))}>Confirmar cancelación</Button></div>}
